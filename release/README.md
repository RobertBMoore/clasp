...
GitHub does not expose draft assets through the public `releases/latest/download` feed, so an exact production-feed update test is impossible while the candidate remains a draft. Immediately after a human publishes the release, the `Published direct release acceptance` workflow pins the immutable workflow SHA associated with the release event, proves both that verifier commit and the release source are ancestors of the protected default branch, and compares anonymous downloads of the public `latest` appcast, release archive, and checksum byte-for-byte with the tagged assets. In archive-only mode, the verifier checks release names, checksum, appcast, and signatures before extracting the public archive into a private temporary directory, then validates the extracted app's Developer ID signature, notarization ticket, Gatekeeper result, bundle metadata, and Sparkle boundary. The acceptance receipt records both the release source SHA and trusted verifier SHA. This is post-publication detection and rollback evidence, not a pre-publication gate: an existing Sparkle client could poll during the acceptance window. Do not announce or intentionally distribute the release until that workflow passes and a clean Mac completes [`DIRECT_UPDATE_ACCEPTANCE.md`](DIRECT_UPDATE_ACCEPTANCE.md) for the real N→N+1 in-app update. If either check fails, immediately unpublish the release and investigate.

If an already-tagged release exposes a defect in its tag-era acceptance harness, first return the release to draft. After the repair is merged to protected `main`, the same workflow has a recovery dispatch that must run only from `main` and requires the exact published tag, numeric release ID, resolved source commit, and human-approved archive SHA-256 captured before publication. The recovery path verifies the live release is public and non-prerelease, requires exactly the expected appcast/archive/checksum asset set and GitHub archive digest, binds its tag and repaired verifier to protected default-branch history, and performs the same anonymous-download and notarized-archive checks before issuing a receipt. It never makes a draft public and does not weaken the normal published-event gate for future tags. Keep the normal event workflow active for an honest audit trail; the known tag-era run may fail only for the already-recorded harness defect, and the release must return to draft immediately if the repaired recovery run does not start and pass before any announcement or intentional distribution.

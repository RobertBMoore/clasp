...
- [x] Compile and test the production Store boundary from a clean scratch path: 87/87 Store tests and 93/93 full local tests pass with zero failures; the Store binary is `arm64`, Sparkle is absent, Accessibility/CGEvent capture symbols are absent, and SwiftPM reports no external Store dependencies.
